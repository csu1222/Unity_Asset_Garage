...
namespace AssetGarage.CombatDemo.Editor.Tests
{
    public sealed class CombatDemoTests
    {
        ...
        [Test] public void RadialGuideFillsUseSharedThresholds(){TimingPresentationMapping.RadialFills(.85f,.925f,out float normal,out float great,out float extreme);Assert.That(normal,Is.EqualTo(.85f).Within(.0001f));Assert.That(great,Is.EqualTo(.075f).Within(.0001f));Assert.That(extreme,Is.EqualTo(.075f).Within(.0001f));Assert.That(normal+great+extreme,Is.EqualTo(1f).Within(.0001f));}
        [Test] public void RadialGuideFillsClampInvalidThresholdOrder(){TimingPresentationMapping.RadialFills(.9f,.8f,out float normal,out float great,out float extreme);Assert.That(normal,Is.EqualTo(.9f));Assert.That(great,Is.Zero);Assert.That(extreme,Is.EqualTo(.1f).Within(.0001f));}
